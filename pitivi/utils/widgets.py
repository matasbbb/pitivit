...
class FractionWidget(TextWidget, DynamicWidget):
        ...
        if presets:
            for preset in presets:
                if type(preset) is str:
                    ...
                    strval = "%g:%g" % (preset.numerator, preset.denominator)
    ...
    def setWidgetValue(self, value):
        ...
        if (value.denominator / 1001) == 1:
            text = "%gM" % (value.numerator / 1000)
        else:
            text = "%g:%g" % (value.numerator, value.denominator)
...
if __name__ == '__main__':
    ...
    widgets = (
        ...
        (FontWidget, "Sans 9", ()),
        (FractionWidget, "30M",
            (FractionRange(Fraction(1, 1),
                Fraction(30000, 1001)),)),
        (FractionWidget, Fraction(25000, 1001),
            (
                FractionRange(
                    Fraction(1, 1),
                    Fraction(30000, 1001)
                ),
                ("25:1", Fraction(30, 1), "30M", ),
            )
        ),
    )
...
class GstElementSettingsDialog(Loggable):
    ...
    def _fillWindow(self):
        ...
        self.window.set_title(_("Properties for %s") % self.factory.get_metadata('long-name'))
