...
## PiTiVi current project callbacks
    ...
    def _settingsChangedCb(self, project, unused_old, new):
        ...
        self.viewer.setDisplayAspectRatio(
            float(new.videopar.numerator / new.videopar.denominator * new.videowidth) /\
...
#Pipeline messages
    ...
    def _busMessageCb(self, unused_bus, message):
        ...
        if message.type == Gst.MessageType.EOS:
            ...
            self.app.current.pipeline.set_state(Gst.State.PAUSED)
...
## other
    ...
    def _timelineSeekRelativeCb(self, unused_seeker, time):
        try:
            position = self.app.current.pipeline.query_position(Gst.Format.TIME)[0]
            position += time

            self.app.current.pipeline.seek(1.0, Gst.Format.TIME, Gst.SeekFlags.FLUSH,
    ...
    def _timelineSeekCb(self, ruler, position, format):
        ...
        try:
            ...
            if not self.app.current.pipeline.seek(1.0, format, Gst.SeekFlags.FLUSH,
                    Gst.SeekType.SET, position, Gst.SeekType.NONE, -1):
